    ...
    initVis() {
        ...
        vis.brush = d3.brushX()
            ...
            .on('brush', function ({selection}) {
                if (selection) vis.brushed(selection,false);
            })
        ...
        // vis.renderLine(vis.detailedView_area, vis.data, vis.xScale_detail, vis.yScale_detail,  true)
        vis.renderLine(vis.overview_area, vis.data, vis.xScale_overview, vis.yScale_overview,  true)
        // Update the brush and define a default position
        const defaultBrushSelection = [vis.xScale_overview(selectedDomain[0]), vis.xScale_overview(selectedDomain[1])];
        vis.brushG
            .call(vis.brush)
    ...
    brushed(selection,if_end=true) {
        ...
        if (selection) {
            ...
            if(if_end){
                ...
                map.filterDateRange(from, end)}
        } else {
            // Reset x-scale of the focus view (full time period)
            selectedDomain = vis.xScale_overview.domain()
            vis.xScale_detail.domain(vis.xScale_overview.domain());
            if(if_end){
                map.filterDateRange(vis.xScale_overview.domain()[0], vis.xScale_overview.domain()[1])}
        }
        ...
        // Redraw line
        vis.renderLine(vis.detailedView_area, vis.data, vis.xScale_detail, vis.yScale_detail, lineChart===undefined)
...
// scales and if need to label the name
    renderLine(area, data, x_scale, y_scale, if_animation) {
        let vis = this
    ...
    update_Title_and_AxisName() {
        ...
        vis.svg.select('#y-axis-name')
            .text('Counts/Street \r in one Hour')
            .attr("x", vis.margin.left+50)
        vis.svg.select('#linechart_title')
            ...
            .text("Average Vehicle Count for the New York Sate"+ " From " + selectedDomain[0].toDateString() + " to " + selectedDomain[1].toDateString())
    }
