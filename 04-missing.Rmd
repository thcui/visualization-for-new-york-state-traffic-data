...
create_missing_plots(df_final_project,percent=FALSE,max_patterns =15) + 
  ggtitle("Missing Patterns for the Transformed Dataset") +
  theme(plot.title = element_text(hjust = 0.7, size = 40))
```
...
df_final_project <- df_final_project %>%
  filter(REGION=='11'& COUNTY_CODE=='1'& `MONTH`=='11')%>%
...
ggplot(df_final_project, aes(x = key, y = `Date`, fill = missing)) + 
  geom_raster() + scale_fill_manual(values = c("white", "red"))+scale_y_date(date_breaks = "1 day") + theme_bw()+ facet_wrap(vars(
COUNT_ID),scales = "free",ncol=3)+ggtitle("Missing data for each count session in November, 2020 from New York County in New York city")+xlab('Time Period')+ylab('Date')
```
